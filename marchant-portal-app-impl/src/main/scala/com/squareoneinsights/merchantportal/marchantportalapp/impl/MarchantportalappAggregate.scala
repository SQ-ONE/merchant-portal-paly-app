...
/**
  * This provides an event sourced behavior. It has a state, [[MarchantportalappState]], which
  ...
  */
object MarchantportalappBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[MarchantportalappCommand]): Behavior[MarchantportalappCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, MarchantportalappEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[MarchantportalappCommand, MarchantportalappEvent, MarchantportalappState](
        persistenceId = persistenceId,
        emptyState = MarchantportalappState.initial,
...
/**
  ...
  */
case class MarchantportalappState(message: String, timestamp: String) {
  def applyCommand(cmd: MarchantportalappCommand): ReplyEffect[MarchantportalappEvent, MarchantportalappState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: MarchantportalappEvent): MarchantportalappState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[MarchantportalappEvent, MarchantportalappState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[MarchantportalappEvent, MarchantportalappState] =
    Effect
...
object MarchantportalappState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: MarchantportalappState = MarchantportalappState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[MarchantportalappCommand]("MarchantportalappAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[MarchantportalappState] = Json.format
}

/**
  * This interface defines all the events that the MarchantportalappAggregate supports.
  */
sealed trait MarchantportalappEvent extends AggregateEvent[MarchantportalappEvent] {
  def aggregateTag: AggregateEventTag[MarchantportalappEvent] = MarchantportalappEvent.Tag
}

object MarchantportalappEvent {
  val Tag: AggregateEventTag[MarchantportalappEvent] = AggregateEventTag[MarchantportalappEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MarchantportalappEvent
...
/**
  ...
  */
trait MarchantportalappCommandSerializable

/**
  * This interface defines all the commands that the MarchantportalappAggregate supports.
  */
sealed trait MarchantportalappCommand
    extends MarchantportalappCommandSerializable
...
case class UseGreetingMessage(message: String, replyTo: ActorRef[Confirmation])
    extends MarchantportalappCommand
...
/**
  ...
  * application loader.
  */
object MarchantportalappSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MarchantportalappState],
