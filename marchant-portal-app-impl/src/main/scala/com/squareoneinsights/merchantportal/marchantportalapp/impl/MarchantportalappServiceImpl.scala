...
import com.squareoneinsights.merchantportal.marchantportalapp.api
...
/**
  * Implementation of the MarchantportalappService.
  */
class MarchantportalappServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends MarchantportalappService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[MarchantportalappCommand] =
    clusterSharding.entityRefFor(MarchantportalappState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(MarchantportalappEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[MarchantportalappEvent]
